    ...
    try:
        ...
        if "Attributes" not in result:
            return {
                "statusCode": 404,
                "body": json.dumps(
                    {"status": "not_found", "message": "Item não existia na tabela."},
                    ensure_ascii=False,
                ),
            }
        else:
        ...
        return {
            "statusCode": 500,
            "body": json.dumps(
                {"status": "error", "message": f"Tabela não encontrada: {str(e)}"},
                ensure_ascii=False,
            ),
        }
        ...
        return {
            "statusCode": 500,
            "body": json.dumps(
                {"status": "error", "message": f"Erro ao excluir item: {str(e)}"},
                ensure_ascii=False,
            ),
        }
